...
- `refresh_token` 用于已保存账号登录和 access token 续期，客户端使用它换取新的 `access_token`。
...
- `refresh_token` 只在主动退出登录、`session_only` 客户端关闭清理、同一用户重新账号密码登录替换旧 token，或 Redis TTL 自然到期时失效。
- 关闭保存了自动登录凭据的客户端、断网、崩溃、WebSocket 断开、`access_token` 过期，都不删除已保存的 `refresh_token`。
...
| `refresh_token` | 长期随机 opaque token | 已保存账号登录和 access token 续期 | 自动登录时持久化到客户端系统凭证；未选择自动登录时不持久化；服务端 Redis 保存 hash | 用户主动 logout 删除、`session_only` 客户端关闭清理、同一用户重新账号密码登录替换，或 Redis TTL 自然到期 |

`access_token` 过期后，客户端应调用 `/v1/auth/refresh` 获取新的 `access_token`。如果 refresh token 仍有效，客户端不需要重新输入账号密码。客户端启动时先加载已保存用户列表；用户选择某个已保存账号后，客户端再读取该用户的本地 refresh token 并调用 `/v1/auth/refresh`。
...
| `refresh_token` | `string` | 是 | 长期随机 opaque refresh token。客户端根据自动登录选择写入系统凭证；未选择自动登录时不持久化，仅由当前运行时会话持有。 |
...
6. 查询 Redis service registry，选择可用 `im-chat` 节点。
7. 如果没有可用 `im-chat` 节点，返回 `no_available_chat_node`，不返回新 token。
8. 计算 `refresh_token` hash，并写入 Redis `refresh_token:{token_hash}`，设置 TTL。
9. 写入 Redis `refresh_token_by_user:{user_id}`，value 为新的 refresh token hash，TTL 与主记录对齐。
10. 返回 `user_id`、token 和 `im_chat_ws_url`。

## `POST /v1/auth/refresh`

### 用途

客户端使用当前会话或本地保存的 `refresh_token` 换取新的短期 `access_token`。对于已保存账号登录，客户端先展示本地已保存用户列表；用户选择账号后，客户端读取该用户保存的 refresh token 并调用本接口。
...
| 字段名 | 类型 | 是否必填 | 字段含义 |
| --- | --- | --- | --- |
| `refresh_token` | `string` | 是 | 客户端当前会话或本地保存的 refresh token 明文。 |
...
| 字段名 | 类型 | 是否必填 | 字段含义 |
| --- | --- | --- | --- |
| `refresh_token` | `string` | 是 | 客户端当前会话或本地保存的 refresh token 明文。服务端用它计算 hash，并删除对应 Redis 认证状态。 |
...
`/v1/auth/logout` 用于主动退出登录，也用于 `session_only` 会话在客户端关闭前的 best-effort 清理。关闭保存了自动登录凭据的客户端、断网、崩溃和 WebSocket 断开不删除已保存 refresh token。
...
- refresh token 只因 logout、`session_only` 客户端关闭清理、同一用户重新账号密码登录替换，或 Redis TTL 到期失效。
