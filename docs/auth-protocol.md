...
- `refresh_token` 只在主动退出登录或 Redis TTL 自然到期时失效。
...
| `refresh_token` | 长期随机 opaque token | 免输入账号密码换取新的 `access_token` | 客户端本地安全存储；服务端 Redis 保存 hash | 用户主动 logout 删除，或 Redis TTL 自然到期 |
...
Redis key 使用 `refresh_token:{token_hash}`。该 key 的边界已在 `docs/redis-routing-presence.md` 中定义；本文档只定义认证侧 value 字段。
...
1. 校验账号和密码格式。
2. 校验账号密码是否正确。
3. 签发短期 JWT `access_token`。
4. 生成长期随机 `refresh_token`。
5. 计算 `refresh_token` hash，并写入 Redis `refresh_token:{token_hash}`，设置 TTL。
6. 查询 Redis service registry，选择可用 `im-chat` 节点。
7. 返回 token 和 `im_chat_ws_url`。
...
9. 返回新的 `access_token`、过期时间和 `im_chat_ws_url`。
...
| 字段名 | 类型 | 是否必填 | 字段含义 |
| --- | --- | --- | --- |
| `refresh_token` | `string` | 是 | 客户端本地保存的 refresh token 明文。服务端用它计算 hash 并删除 Redis key。 |
...
4. 无论 key 是否已不存在，只要删除操作本身成功，都可以返回 `204 No Content`。
5. 客户端删除本地 `access_token` 和 `refresh_token`。
6. 客户端主动关闭 WebSocket；连接断开后的 presence 清理由 `docs/redis-routing-presence.md` 约束。
...
| --- | --- | --- | --- |
| 登录成功 | 签发新的短期 JWT | 签发新的随机 token，并写入 Redis hash | 不自动在线，客户端仍需连接 WebSocket 并 auth |
| `/v1/auth/refresh` 成功 | 签发新的短期 JWT | 保持原 token，不轮换、不删除 | 不自动在线，客户端仍需连接或重连 WebSocket |
| `/v1/auth/logout` 成功 | 客户端删除本地 token | 服务端删除 Redis hash，客户端删除本地 token | 客户端应关闭 WebSocket；presence 由连接断开流程清理 |
...
- `/v1/auth/login` 返回 `access_token`、`refresh_token`、`access_token_expires_at`、`im_chat_ws_url`。
- `/v1/auth/refresh` 返回 `access_token`、`access_token_expires_at`、`im_chat_ws_url`，不返回 `refresh_token`。
- `/v1/auth/refresh` 不轮换 refresh token，不删除旧 refresh token。
- `/v1/auth/logout` 删除 Redis `refresh_token:{token_hash}`。
- refresh token 只因 logout 或 Redis TTL 到期失效。
