...
const MESSAGE_CREATED: &str = "message_created";
...
async fn insert_outbox_event(
    ...
    let aggregate_id = message.conversation_id.to_string();
    let payload = serde_json::json!({
        ...
        "occurred_at": format_protocol_time(occurred_at),
        "message": message,
    });
...
mod tests {
    ...
    async fn accept_message_writes_message_and_outbox_when_test_database_is_configured() {
        ...
        let outbox_count: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM outbox_events WHERE type = ? AND aggregateid = ?",
        )
        .bind(MESSAGE_CREATED)
    ...
    async fn test_pool() -> Option<MySqlPool> {
        let Ok(database_url) = env::var("TEST_DATABASE_URL") else {
            eprintln!("skip MySQL integration test: TEST_DATABASE_URL is not set");
            return None;
        };
        Some(
