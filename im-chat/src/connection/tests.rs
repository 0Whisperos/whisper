...
fn replace_if_match_restores_connection_when_expected_connection_is_current() {
    ...
    let replaced =
        registry.replace_if_match(20001, "current", test_connection(20001, "replacement"));
...
fn replace_if_match_skips_when_expected_connection_is_stale() {
    ...
    let replaced = registry.replace_if_match(20001, "stale", test_connection(20001, "replacement"));
...
async fn send_to_connection_delivers_message_when_connection_id_matches() {
    ...
    let result = registry.send_to_connection(20001, "connection-1", Message::Text("hello".into()));
...
async fn send_to_connection_rejects_mismatched_connection_id() {
    ...
    let result = registry.send_to_connection(20001, "old", Message::Text("hello".into()));
...
#[test]
fn reports_full_and_closed_queues_without_waiting() {
    // 测试目标：验证满队列和关闭队列立即返回明确结果，不阻塞消费循环。
    // 构造方法：用容量一的队列注册连接，填满后再次投递，再关闭接收端。
    // 输入数据：同一 user_id/connection_id 连续投递三条消息。
    // 预期行为：依次返回 Sent、Full、Closed。
    let registry = ConnectionRegistry::new();
    let (sender, receiver) = mpsc::channel(1);
    let mut connection = test_connection(20001, "connection-1");
    connection.sender = sender;
    registry.insert(connection);
    assert_eq!(
        registry.send_to_connection(20001, "connection-1", Message::Text("first".into())),
        SendToConnectionResult::Sent
    );
    assert_eq!(
        registry.send_to_connection(20001, "connection-1", Message::Text("second".into())),
        SendToConnectionResult::Full
    );
    drop(receiver);
    assert_eq!(
        registry.send_to_connection(20001, "connection-1", Message::Text("third".into())),
        SendToConnectionResult::Closed
    );
}

#[test]
fn reconnect_prevents_old_route_from_sending_to_new_connection() {
    // 测试目标：验证连接替换后，旧 presence 的连接 ID 无法向新连接投递。
    // 构造方法：先注册旧连接再覆盖为新连接，分别使用旧 ID 和新 ID 投递。
    // 输入数据：user_id=20001，connection_id 从 old 变成 new。
    // 预期行为：旧 ID 返回不匹配，新 ID 成功且新队列仅收到一条消息。
    let registry = ConnectionRegistry::new();
    registry.insert(test_connection(20001, "old"));
    let (sender, mut receiver) = mpsc::channel(1);
    let mut connection = test_connection(20001, "new");
    connection.sender = sender;
    registry.insert(connection);
    assert_eq!(
        registry.send_to_connection(20001, "old", Message::Text("stale".into())),
        SendToConnectionResult::ConnectionIdMismatch
    );
    assert_eq!(
        registry.send_to_connection(20001, "new", Message::Text("current".into())),
        SendToConnectionResult::Sent
    );
    let Message::Text(text) = receiver.try_recv().expect("new message") else {
        panic!("text message");
    };
    assert_eq!(text.as_str(), "current");
    assert!(receiver.try_recv().is_err());
}
